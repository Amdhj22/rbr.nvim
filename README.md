...
`rbr.nvim` paints Neovim with the [RBR color scheme](https://github.com/Amdhj22/rbr) — **kerb red** for what's active, **RB yellow** for what needs attention, everything else stepping back in pastel. Colors live upstream in `rbr-theme`'s `palette.json` and are vendored here as a single Lua table regenerated by a sync script.
